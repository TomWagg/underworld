...
def enter_the_underworld(n_binaries, output_dir, processes, simulation_name, file_suffix):
    ...
    start = time.time()
    initial_pop.save(join(output_dir, f"template{file_suffix}"), overwrite=True)
    ...
    start = time.time()
    binary_underworld.save(join(output_dir, f"{simulation_name}{file_suffix}"), overwrite=True)
